    ...
    onSubmit: async (values) => {
      ...
      if (allNonOptionalFieldsNotEmpty) {
        ...
        !isEnableEdit
          ? // add new data
            await api
              .post('external_dissemination/insert', values)
              .then((response) => {
          ...
          : // update data
            await api
              .put('external_dissemination/update/' + editId, values)
              .then((response) => {
                toast.success(response.data.message)
                fetchData()
                setValidated(false)
        ...
        setSelectRequired('External Document is required')
        setValidated(true)
      }
    },
  })
    ...
    },
    {
      ...
      accessorFn: (row) => {
        if (row.date_received) {
          const originalDate = new Date(row.date_received)
          const options = {
            year: 'numeric',
            month: '2-digit',
            day: '2-digit',
      ...
      <CCard className="mb-4">
        ...
        <CCardBody>
          <MaterialReactTable
            ...
            initialState={{
              density: 'compact',
              columnPinning: { left: ['mrt-row-actions', 'external_log_number'] },
            }}
                  ...
                  <IconButton
                    ...
                    onClick={() => {
                      ...
                      formik.setValues({
                        ...
                        date_received: row.original.date_received,
                      ...
                      Swal.fire({
                        ...
                        if (result.isConfirmed) {
                          validationPrompt(() => {
                            ...
                            api
                              .delete('external_dissemination/delete/' + id)
                              .then((response) => {
                                fetchData()
                                toast.success(response.data.message)
                              })
                ...
                <CFormLabel>
                  {
                    <>
                      ...
                      {requiredField(' External Document')}
                    </>
                  }
                </CFormLabel>
                <Select
